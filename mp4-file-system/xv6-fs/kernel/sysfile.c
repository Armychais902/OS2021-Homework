...
sys_open(void)
{
  ...
  // if O_NOFOLLOW is set, process normally
  if (ip->type == T_SYMLINK && !(omode & O_NOFOLLOW))
  {
    ...
    while (ip->type == T_SYMLINK)
    {
      ...
      // read out the file linked to
      int len = 0;
      memset(sympath, 0, MAXPATH + 1);
      readi(ip, 0, (uint64)&len, 0, sizeof(int));
      readi(ip, 0, (uint64)sympath, sizeof(int), len + 1);
      // switch to another ip, lock release
      iunlockput(ip);
      // get the next inode by path name
...
sys_symlink(void)
{
  ...
  begin_op();
  // what if the path already exist? create return 0, fail
  // create with lock!
