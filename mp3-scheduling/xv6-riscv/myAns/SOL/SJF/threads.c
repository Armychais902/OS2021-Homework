...
void schedule(void){
    /* SJF */
    // call schedule before thread_exit free(), so need to consider is_exit
    if (is_thread_start == 0 || current_thread->is_exited == 1 || current_thread->is_yield ==1)
    {
        // if is_exited and only one thread remain, it will not call schedule()
        // so don't need to worry minthrd becomes NULL
        struct thread *tmp_thread = current_thread->next;
        struct thread *minthrd = (current_thread->is_exited == 1)?NULL:current_thread;
        int mint = (current_thread->is_exited == 1)?__INT32_MAX__:current_thread->remain_execution_time;
        while (tmp_thread != current_thread)
        {
            // not sure if is_exited happens,
            // to my understanding, if the flag is on, it should be removed right away
            // however, still preserve this if-condition
            if (tmp_thread->is_exited == 1)
            {
                tmp_thread = tmp_thread->next;
                continue;
            }
            if (tmp_thread->remain_execution_time < mint || (tmp_thread->remain_execution_time == mint && tmp_thread->ID < minthrd->ID))
            {
                mint = tmp_thread->remain_execution_time;
                minthrd = tmp_thread;
            }
            tmp_thread = tmp_thread->next;
        }
        current_thread = minthrd;
    }
}
